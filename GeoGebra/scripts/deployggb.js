...
//            appletElem.appendChild(resource3);
    ...
    function continueInit() {
        ...
        if (html5OverwrittenCodebaseVersion !== null) {
            ...
            if (parseFloat(html5Version) < 4.4) { // Use 5.0 as default for html5. Change the version number here, when a new stable version is released.
                html5Version = "4.4";
            }

            if (parseFloat(html5Version) < 5.0) {
                if (isFlexibleWorksheet()) {
                    log("force 5.0 codebase for flexible worksheets");
                    html5Version = "5.0";
                } else if (valBoolean(parameters.showToolBar)) {
                    html5Version = "5.0"; // force 5.0 for all applets using the toolbar
                }
            }
